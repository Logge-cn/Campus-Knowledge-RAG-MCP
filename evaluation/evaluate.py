...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset", type=Path, default=EVALUATION_PATH)
...
def _metrics(cases: list[dict], rankings: dict[str, list[list[int]]], chunks: list[dict], timings: dict[str, list[float]]) -> dict:
    ...
    for method, method_rankings in rankings.items():
        ...
        for case_index in answerable:
            ...
            ranks.append(min(relevant_ranks) if relevant_ranks else None)
        ...
        output[method] = {
            ...
            "recall_at_1": round(sum(rank == 1 for rank in ranks) / len(answerable), 4),
            "recall_at_3": round(sum(rank is not None and rank <= 3 for rank in ranks) / len(answerable), 4),
            "recall_at_5": round(sum(rank is not None and rank <= TOP_K for rank in ranks) / len(answerable), 4),
            "mrr": round(mean(1 / rank if rank is not None and rank <= TOP_K else 0.0 for rank in ranks), 4),
