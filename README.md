...
<p><strong>把本地校园 PDF 转换成可追溯、可复现的证据检索服务</strong></p>
...
校园文档 RAG-MCP 面向需要在本地检索中文 PDF 的 MCP 客户端。它返回带文件名、页码和稳定 chunk ID 的证据，并显式判断当前证据是否足以支持回答。

项目负责文档处理、检索和证据充分性判断；最终自然语言答案仍由 Codex 或其他 MCP 客户端生成。证据级 Recall 不等于最终答案正确率。

## 项目一览

| 真实文档 | 处理规模 | 当前索引 | 历史 v2 验收集 |
| ---: | ---: | ---: | ---: |
| 2 份 PDF | 393 页 | 731 chunks | Recall@5 100% |

- 索引 schema 为 4；历史 v2 验收集 Recall@1 / MRR@5 为 90.00% / 94.37%。
- 2026-08-15 已完成 160 DPI 干净环境复现：MCP、冻结周期 96 项测试、两组证据门禁和性能基准均通过。
- 答案级评测基础设施已经完成；真实 12 题答案生成和人工复核尚未执行。
- 演示也可以[下载后离线打开](data/demo/index.html)。
